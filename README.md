...
*   Go 1.22 以上をインストールしてください。
*   MySQL を使う場合は、任意のデータベースを作成して `init.sql` を適用してください。

### 2. 環境変数

*   `PORT` (任意): サーバー待受ポート。未設定の場合は `8000`。
*   `API_KEYS` (必須: API を呼び出す場合): 20 文字の 16 進数キーをカンマ区切りで指定します。
    *   例: `API_KEYS=0123456789abcdef0123,abcdef0123456789abcd`
*   `DATABASE_DSN` (任意): MySQL を使う場合に設定します。
    *   例: `DATABASE_DSN="user:password@tcp(127.0.0.1:3306)/paper_street?parseTime=true"`

### 3. サーバー起動

```bash
go run ./cmd/paper-street-server
```
