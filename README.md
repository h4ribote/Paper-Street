...
* **個別サービス（現状の Compose 構成）**: コンテナごとにプロセスが分かれるため、停止・再起動やスケールが簡単ですが、各コンテナのランタイムやログ周りのオーバーヘッド分だけメモリが増えます。
* **1つのコンテナで全ボット**: コンテナのオーバーヘッドが減るため **メモリ使用量は小さくなる傾向**があります。一方で、1プロセスの障害が全ボット停止につながりやすく、運用上の分離性は下がります。
