...
初期フェーズ（MVP）では、完全な機械学習モデルではなく、**高度なルールベースと状態遷移（State Machine）**によって「学習しているかのような振る舞い」を模倣します。
...
*   **将来的な拡張 (Post-MVP)**: 取引データを蓄積し、強化学習（Reinforcement Learning）を用いたモデルの導入を検討します。
