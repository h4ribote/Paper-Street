...
10. **Yield Hunters (利回り狙い - 指値特化)**:
    *   国債や高配当株などのインカムゲインを狙うボットです。
    *   相対的な利回りが高い資産へ資金を移動させます。
...
#### アルゴリズムボットの通貨スペシャライゼーション
商品の取引（Commodity Trading）に集中させるため、Momentum ChaserやDip Buyerなどのアルゴリズムボットは、それぞれ3つの通貨グループに分かれて資金を保有します。
これにより、各ボットは特定の地域や産業セクターに関連する商品取引に特化します。

1.  **Group A (Northern / Eastern Mix)**
    *   **保有通貨**: [**ARC**, **BRB**, **DRL**]
    *   **役割**: 基軸通貨ARCと、資源・重工業に関連するBRB, DRL建ての商品の裁定やトレンド形成を行います。
2.  **Group B (Oceanic / Southern Agri)**
    *   **保有通貨**: [**VND**, **VDP**]
    *   **役割**: 金融都市VNDと農業国VDPの通貨を持ち、食料品や金融商品のボラティリティに対応します。
3.  **Group C (Energy / Industrial Zone)**
    *   **保有通貨**: [**ZMR**, **RVD**]
    *   **役割**: エネルギー資源ZMRと製造業RVDの通貨を持ち、エネルギーコストと製品価格の連動性を監視します。

各ボットは初期状態でこれらの通貨を物理的に保有（例: 各200,000〜300,000単位）しており、両替を行うことなく即座にコモディティ市場に参加できます。
...
*   **地域スペシャライゼーション**:
    *   機関投資家（Whale）も、アルゴリズムボットと同様に**通貨ブロック（地域）**ごとに専門化されています（[Northern/Eastern], [Oceanic/Southern], [Energy/Industrial]）。
    *   これにより、各経済圏内での大規模な実需取引（サプライチェーンの上流から下流への資金移動など）や、関連銘柄間の裁定取引を主導します。
*   **挙動**:
...
*   **挙動**:
    ...
    *   **金利ニュースへの反応**:
        *   **利上げ (Rate Hike)**: 債券価格の下落を予測し、保有する永久債（Consols）を即座に成行で売却（ショート）します。
        *   **利下げ (Rate Cut)**: 債券価格の上昇を予測し、永久債を即座に成行で購入します。
...
### 3.10. Yield Hunter Logic (利回り狙い)
*   **目的**: インカムゲイン（利息・配当）を最大化するために、相対的に利回りの高い資産に資金を移動させる。
*   **挙動**:
    *   **利回り計算**: 常に全銘柄の「現在利回り」を監視します。
        *   永久債: `Base Coupon / 現在価格`
        *   株式: `予想配当額 / 現在価格`
    *   **スイッチング (Asset Switching)**:
        *   債券の利回りが株式の配当利回り（＋リスクプレミアム）を上回った場合、保有する株式を売却し、国債の買い板に指値注文を置きます。
        *   逆に株式の利回りが魅力的な水準まで上昇（株価下落）した場合は、債券を売って株を拾います。
    *   **役割**: 異なる資産クラス（Asset Classes）間の価格調整機能（裁定）を担い、市場全体のバランスを保ちます。

### 3.11. The Public / Mass Consumer Logic (大衆消費者)
*   **目的**: 経済活動の終着点として、市場からコモディティを購入し、消費（Burn）する。
*   **資金源**: 企業からの給与（Payroll）および国家からの給付金（UBI）。
*   **挙動**:
    *   **Market Buy**: 定期的に、生活必需品（食料、エネルギー、日用品）や高級品を市場価格で購入します。
    *   **Burn**: 購入したアセットは即座に焼却（Burn）され、市場から消滅します。
    *   **需要変動**:
        *   資金潤沢時（好景気）: 高級品（Luxury Goods）やハイテク製品への支出を増やします。
        *   資金不足時（不況）: 生活必需品のみを購入し、高級品需要が蒸発します。

### 3.12. Corporate AI Logic (企業AI - Procurement)
*   **目的**: 事業活動に必要な資源を調達し、[Corporate Contracts](./CONTRACTS.md) を発行する。
*   **挙動**:
    *   **Contract Issuance**: 設備投資（CapEx）や在庫補充が必要になった際、市場価格より高いプレミアムを乗せて大規模な買取オファー（Contract）を提示します。
    *   **自社株買い**: 内部留保が過剰になった場合、株価維持のために自社株買いを行うことがあります。
