...
ユーザーのロング/ショートはこのプールから「在庫」を引き出すことで実行され、在庫の枯渇度合い（利用率）に応じて、**金利（ロング用コスト）**と**貸出料（ショート用コスト）**がリアルタイムに変動します。なお、エントリー時の取引で取得した株や通貨などのアセットは、担保として拘束されるのみで、プールの供給量（`total_cash` / `total_assets`）には加算しません。

### シーソー・メカニズム (The Seesaw Balance)

ユーザーのアイデアにある「ショートすることで比率が戻る（金利が下がる）」という挙動は、本設計では採用せず、ロング/ショートの金利・貸出料はそれぞれの利用率のみで決まります。
...
* **重要**: ショートで得た現金（売却代金）は担保として拘束されるが、プールの貸出可能現金には加算しない。
* **結果**: プールの「株」が減る。「現金」は増えない。
* **影響**: **買方金利は下落せず**、株不足により**貸出料が上昇**する。
...
| パラメータ | 記号 | 日次 (bps) | 説明 |
| --- | --- | --- | --- |
| **Base Rate** | $R_{base}$ | **0.10%** (10) | 利用率0%の時の最低金利。 |
| **Multiplier** | $R_{slope1}$ | **0.40%** (40) | 通常時の金利上昇係数。 |
| **Jump Multiplier** | $R_{slope2}$ | **5.00%** (500) | 枯渇時のペナルティ金利係数。 |
| **Kink Point** | $U_{optimal}$ | **70%** | 金利が跳ね上がる利用率の閾値。 |

#### 計算式

1. **通常時 ($U \le U_{optimal}$)**<br>
   $$R = R_{base} + R_{slope1} \times \frac{U}{U_{optimal}}$$

2. **枯渇時 ($U > U_{optimal}$)**<br>
   $$R = R_{base} + R_{slope1} + R_{slope2} \times (U - U_{optimal})$$

#### 具体例

*   **利用率 50%**: $`10 + 40 \times (0.50 / 0.70) = 38.57\dots \approx 38.6 bps = 0.386\% / day`$
*   **利用率 60%**: $`10 + 40 \times (0.60 / 0.70) = 44.28\dots \approx 44.3 bps = 0.443\% / day`$
*   **利用率 90%**: $`10 + 40 + 500 \times (0.90 - 0.70) = 10 + 40 + 100 = 150 bps = 1.50\% / day`$
*   **利用率 100%**: $`10 + 40 + 500 \times (1.00 - 0.70) = 10 + 40 + 150 = 200 bps = 2.00\% / day`$

*   **徴収タイミング**: **2時間ごと**にポジションから徴収されます。 ($`\frac{R}{12}\% / 2hour`$)
*   **UI表示**: 「年利 (APY)」ではなく、**「日次金利 (Daily Rate)」** や **「Borrow Fee: 0.10% / day」** と表示します。
...
4. **担保拘束 (Collateral Lock)**:
* 売却代金 500 ARC は、ショートポジションの担保としてプール内に留保される。
* **ここがポイント**: これを「貸出可能現金」として扱わず、`total_cash` を増加させない。
* 現金利用率 $`U_{cash}`$ は変化せず、**ロング金利は下落しない**。
...
1. **「逆張り」インセンティブの発生**:
* ロングが殺到して金利が高騰している時、ショートを入れると「高いロング金利を払わずに済む」メリットが生まれます（ただしショートによる現金供給は行わないため、ロング金利自体は低下しません）。
...
**引き出し可能額の制限**:
* 引き出しは利用率が **100% を下回る範囲** でのみ可能です。プールに余力（`total_cash - borrowed_cash`）がない場合は引き出せません。
* 例: ある提供者が 200 ARC を貸し出していて、プールの余力が 100 ARC の場合、引き出せるのは **最大 100 ARC まで** です。
