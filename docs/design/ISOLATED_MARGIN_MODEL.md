...
`DUAL_LIQUIDITY_INVENTORY.md` で定義された変動金利は、以下のように処理されます。

1. **証拠金からの控除**:
* 金利（Borrow Rate / Short Fee）は、ウォレット残高からではなく、**ポジションの証拠金 (`margin_used`) から直接控除**する形、または**累積損失として計算式に加算**する形で処理します（本仕様では後者を採用）。
...
## 6. UIへの要件 (UI Requirements)

この仕様変更に伴い、`GAMEPLAY_AND_UI.md` の設計に以下の要素を追加する必要があります。

1. **注文画面**:
* 「レバレッジスライダー」の横に、**「推定ロスカット価格 (Est. Liq Price)」** を表示する。
...
3. **証拠金追加ボタン**:
* 危険なポジションに対して `+ Add Margin` できるボタンを配置。
