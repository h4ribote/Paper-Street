...
*   同一ユーザーの買い注文と売り注文がマッチングする場合、**既存の指値注文（Maker側）をキャンセル**します。
*   成行注文（Market）の場合は、キャンセルされた指値注文の数量分だけ**成行注文のサイズを減少**させます。
...
*   すべての価格と数量は、浮動小数点数（float/double）ではなく、**固定小数点の整数（Decimal/Integer）** として扱われます。
    *   例: 1.000000 は `1000000` (scale=6) として内部処理されます。
...
### 6.1. シーケンシャル処理
*   特定の銘柄（Symbol）に対する注文処理は、**銘柄専用の1つのOrderBook Goroutine**で完全に順序化（Serialized）されます。
*   API層は `Order` を `OrderChannel`（キュー）へ投入し、OrderBookワーカーが1件ずつ順番に処理します。これにより、Mutexなしでレースコンディションを防ぎ、確定的な結果を保証します。

### 6.2. 楽観的ロックの排除
*   マッチングエンジン内部では、DBへの同期書き込みを行わず、**Goプロセス内メモリの板（Bids/Asks）**のみを更新します。
*   約定結果の永続化（MySQLへの書き込み）は、別のDB書き込み専用Goroutine（ワーカープール）で非同期処理します。

### 6.3. 配信経路
*   約定結果と板更新は、OrderBookワーカーからWebSocket配信層へ直接連携します。
*   Redis Pub/Subは利用しません。

### 6.4. 復旧と安全停止
*   **起動時ウォームアップ**: サーバー起動時に `orders` テーブルの `status IN ('OPEN', 'PARTIAL')` を読み込み、インメモリ板を再構築します。
*   **Graceful Shutdown**: シャットダウンシグナル受信時は、DB書き込みキューが空になり全ワーカー完了するまで待機してからプロセス終了します。

## 7. エラーハンドリング (Error Handling)

*   **残高不足 (Insufficient Funds/Balance)**:
    *   注文をOrderChannelへ投入する前に、必要な資金（Buy）または資産（Sell）のロック（Hold）を**MySQLトランザクションで同期的に**行います。
