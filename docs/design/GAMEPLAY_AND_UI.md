...
*   **「再起」ローン (Loan Sharks)**: 高金利ですが、破産状態からでも借り入れ可能な「闇金」機能を提供します。ただし、返済するまで一部機能制限などのペナルティが伴います。
...
*   **Limit (指値)**: 指定価格で待つ。流動性供給者（Maker）として手数料リベートを得られる機能は **Post-MVP** での実装を予定しています。
    *   **FX市場における特例 (Post-MVP)**: FXでは指値注文は「指定価格帯（レンジ）への集中流動性提供」として扱われます。
