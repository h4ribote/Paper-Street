...
| --- | --- | --- | --- |
| [FX_MARKET_MODEL.md](./design/FX_MARKET_MODEL.md) | 集中流動性・Tick・マルチホップ・Fee Tier 分割 | `internal/api/liquidity.go` の `SwapPool` は **入出金 + 手数料控除のみ**。Tick/価格帯計算やルーター探索は未実装。 | 未実装 (簡易Swapのみ) |
...
- 上記は「バックエンドで未実装または簡易実装」と確認できた項目のみを記載しています。
- 既に実装済みの項目（例: マッチングエンジン / 基本API / デイリーミッションのペア達成ロジックなど）は本メモから除外しています。
